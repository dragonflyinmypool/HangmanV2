<script>
  ...
  function updateLives (e) { 
    // Update lives 
    state.livesLeft = state.wordTogether.includes(e.detail.letter) ? state.livesLeft : state.livesLeft - 1;
...
<main>
  ...
  <Menu on:newGame={newGame} {settings} bind:newGameButton={newGameButton}/>
</main>
